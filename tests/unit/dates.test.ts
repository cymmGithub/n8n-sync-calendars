	...
	describe('convertTicksToDate', () => {
		...
		it('should convert ticks to correct milliseconds', () => {
			...
			const expectedMs = Number(
				(testTicks - BigInt(EPOCH_TICKS_AT_UNIX_EPOCH)) /
	...
	describe('isoToTicks', () => {
		...
		it('should be reversible with convertTicksToDate', () => {
			...
			const originalDate = new Date(
				originalIsoString.replace(/[+-]\d{2}:\d{2}$|Z$/, ''),
			);
			expect(
				Math.abs(dateBack.getTime() - originalDate.getTime()),
			).toBeLessThan(1000); // Within 1 second
		});
	});

	describe('getReservationsFromNowUrl', () => {
		beforeEach(() => {
			// Set environment variable for testing
			process.env.OPONEO_RESERVATIONS_LIST_URL =
		...
		it('should generate URL with current timestamp', () => {
			...
			const msFromTicks = Number(
				(ticks - BigInt(EPOCH_TICKS_AT_UNIX_EPOCH)) /
					BigInt(TICKS_PER_MILLISECOND),
			);
	...
	describe('getTimeSlotIndex', () => {
		...
		describe('Error handling', () => {
			it('should throw error when timeString is missing', () => {
				const date = new Date('2025-10-20T14:00:00');
				expect(() => getTimeSlotIndex(null, date)).toThrow(
					'Both timeString and date are required',
				);
			});

			it('should throw error when timeString is undefined', () => {
				const date = new Date('2025-10-20T14:00:00');
				expect(() => getTimeSlotIndex(undefined, date)).toThrow(
					'Both timeString and date are required',
				);
			});
			...
			it('should throw error when timeString is empty string', () => {
				const date = new Date('2025-10-20T14:00:00');
				expect(() => getTimeSlotIndex('', date)).toThrow(
					'Both timeString and date are required',
				);
			});

			it('should throw error when date is missing', () => {
				expect(() => getTimeSlotIndex('14:00', null)).toThrow(
					'Both timeString and date are required',
				);
			});

			it('should throw error when date is undefined', () => {
				expect(() => getTimeSlotIndex('14:00', undefined)).toThrow(
					'Both timeString and date are required',
				);
			});

			it('should throw error when both parameters are missing', () => {
				expect(() => getTimeSlotIndex(null, null)).toThrow(
					'Both timeString and date are required',
				);
			});
		});
