...
router.post(
	...
	async (req: Request, res: Response): Promise<void> => {
		...
		try {
			...
			logger.info(
				`Accessing reservations with URL: ${reservationsFromNowUrl}`,
			);
			...
			const reservationsData =
				await getAllPagesReservations(page);
			logger.info(
			...
			const detailedReservations: Array<{
				...
				details: Awaited<
					ReturnType<typeof scrapeReservationDetails>
				>;
			}> = [];
			let processed = 0;
			let skipped = 0;
			...
			logger.info(
				'Successfully scraped data after authentication',
				{
...
router.post(
	...
	async (req: Request, res: Response): Promise<void> => {
		...
		if (reservations.length === 0) {
			logger.info(
				'No reservations to process - empty array provided',
			);
			res.json({
				success: true,
				results: [],
		...
		try {
			...
			if (!isAuthenticated) {
				...
				browserPool.markAsAuthenticated();
				logger.info(
					'Authentication successful, starting reservation mutations',
				);
			} else {
				logger.info(
			...
			for (let i = 0; i < reservations.length; i++) {
				...
				const startDate = convertTicksToDate(
					reservation.startDate,
				);
				const endDate = convertTicksToDate(
					reservation.endDate,
				);
				...
				const licencePlate =
					reservation.licencePlate ?? 'WO';
				const phoneNumber =
					reservation.phoneNumber ?? 'WO';
				...
				try {
					...
					// Set end time
					logger.info(
						`Setting end time to: ${endDateHour}`,
					);
					await randomDelay(200, 500);
					await page
						.locator(
							'input[name="DateChoose\\.TimeTo"]',
						)
						.click();
					...
					const isDisabled = (
						await timeSlotLocator.getAttribute(
							'class',
							{ timeout: 10000 },
						)
					)?.includes('disabled');
					...
					try {
						...
						logger.error(
							`Failed to click time slot ${endDateHour}`,
							{ error: clickError },
						);
						throw new Error(
					...
					try {
						await page.waitForSelector(
							'text=Pomyślnie dodano rezerwację',
							{
								timeout: 5000,
							},
						);
						logger.info(
							`Successfully created reservation for ${licencePlate}`,
						);

						// Click the button to go to the reservation page
						await page
							.getByText('Wróć do rezerwacji')
							.click();
			...
			const summary: OperationSummary = {
				...
				success_rate:
					(
						(results.length / reservations.length) *
						100
					).toFixed(2) + '%',
			};
			...
			res.status(500).json({
				...
				error:
					error instanceof Error
						...
						: 'Unknown error',
				details:
					'An error occurred during the mutation process',
...
router.post(
	...
	async (req: Request, res: Response): Promise<void> => {
		...
		if (oponeoReservationIds.length === 0) {
			logger.info(
				'No reservations to obliterate - empty array provided',
			);
			res.json({
				success: true,
				results: [],
		...
		try {
			...
			for (let i = 0; i < oponeoReservationIds.length; i++) {
				const oponeoReservationId =
					oponeoReservationIds[i]!;
				logger.info(
				...
				try {
					if (!oponeoReservationId) {
						throw new Error(
							'MISSING_ID - oponeoReservationId is required',
						);
					}
					...
					await page
						...
						.catch(() => {
							logger.warn(
								'Network idle timeout - continuing with cleanup',
							);
						});
			...
			const summary: OperationSummary = {
				...
				success_rate:
					(
						(results.length /
							oponeoReservationIds.length) *
						100
					).toFixed(2) + '%',
			};

			logger.info(
				'Obliteration process complete:',
				summary,
			);
			...
			res.status(500).json({
				...
				error:
					error instanceof Error
						...
						: 'Unknown error',
				details:
					'An error occurred during the obliteration process',
				partial_results: results,
				errors: errors,
			});
		}
